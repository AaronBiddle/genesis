<script lang="ts">
    ...
    import { getDirectoryContents, createDirectory, deleteDirectory, deleteFile } from './FileOperationsService';
    ...
    export let config: Partial<FileOperationsConfig> = {};
    
    // Combine with default config
    const defaultConfig: FileOperationsConfig = {
        fileType,
        fileExtension: '.json',
        validateFilename: (name) => ({ valid: !!name.trim(), message: 'Filename is required' }),
        dialogTitle: {
            save: 'Save File',
            load: 'Load File',
            delete: 'Delete File'
    ...
    // Merge configs
    $: mergedConfig = { ...defaultConfig, ...config, fileType };
    ...
    async function handleCreateDirectory() {
        ...
        try {
            ...
            const result = await createDirectory(fileType, dirPath);
    ...
    function handleSubmit() {
        ...
        const extension = mergedConfig.fileExtension || '.json';
    ...
    async function executeDelete() {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (deleteType === 'directory' && result.error && result.error.includes('must be empty')) {
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold">
                {#if mode === 'save'}
                    {mergedConfig.dialogTitle?.save || 'Save File'}
                {:else if mode === 'load'}
                    {mergedConfig.dialogTitle?.load || 'Load File'}
                {:else}
                    {mergedConfig.dialogTitle?.delete || 'Delete File'}
                {/if}
            </h2>
            <button 
                on:click={close}
                ...
                {#if mode === 'save'}
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-gray-500 mt-1">
                            {mergedConfig.fileExtension || '.json'} extension will be added automatically if not provided
                        </p>
                    </div>
                {:else}
                    <!-- Files list -->
