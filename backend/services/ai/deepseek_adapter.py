...
from .clients import sync_deepseek_client, async_deepseek_client
...
def sync_generate(model, messages, has_thinking, **kwargs):
    ...
    try:
        ...
        resp = sync_deepseek_client.chat.completions.create(**deepseek_args)
...
# Comment out the old asyncio.to_thread version
...
# New async version using acreate
async def async_generate(model, messages, has_thinking, **kwargs):
    """Asynchronously generates response using the AsyncOpenAI client and acreate."""
    start_time = datetime.datetime.now()
    print(f"[{start_time.isoformat()}] DeepSeek async_generate started for model {model}")

    if not async_deepseek_client:
        print("Error: DeepSeek async client is not initialized. Cannot generate response.")
        return None

    # Prepare DeepSeek API call arguments (same logic as sync version)
    final_messages_deepseek = []
    ...
    try:
        api_call_start = datetime.datetime.now()
        print(f"[{api_call_start.isoformat()}] DeepSeek calling async API (acreate) for model {model}...")
        
        # Use await and acreate with the async client
        resp = await async_deepseek_client.chat.completions.acreate(**deepseek_args)
        
        api_call_end = datetime.datetime.now()
        print(f"[{api_call_end.isoformat()}] DeepSeek async API call finished for model {model}. Duration: {api_call_end - api_call_start}")
        ...
        end_time = datetime.datetime.now()
        print(f"[{end_time.isoformat()}] DeepSeek async_generate finished for model {model}. Total duration: {end_time - start_time}")
        return response_data
        
    except Exception as e:
        error_time = datetime.datetime.now()
        # Use specific exception types if available from openai async errors
        print(f"[{error_time.isoformat()}] Error during DeepSeek async API call for model {model}: {e}")
        print(f"[{error_time.isoformat()}] DeepSeek async_generate finished with error for model {model}. Total duration: {error_time - start_time}")
        return None 